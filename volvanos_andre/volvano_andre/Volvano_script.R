...
getwd()
### dataset
de <- read.delim("DESeq2_All_DEGs_SAMS_x_NonSAMS_ATV10uM_vs_VHC.txt", sep= "\t", header = T, dec = ",")
...
pdf("Volcano_DEGs_SAM_nonSAM_ATV.pdf")
...
### dataset
de <- read.delim("DESeq2_All_DEGs_SAMS_x_NonSAMS_ROS10uM_vs_VHC.txt", sep= "\t", header = T, dec = ",")
...
pdf("Volcano_DEGs_SAM_nonSAM_ROS.pdf")
ggplot(data=de, aes(x=log2FoldChange, y=-log10(pvalue))) +
  labs(title="", 
       x="log2 (Fold-Change)", 
       y="-log10 (p-value)")+
